            ...
            for session in sessions:
                ...
                if fade_trigger_time <= now < dialog_trigger_time and session_id not in self.faded_sessions:
                    if self.transition_controller.state == TransitionState.IDLE:
                        ...
                        self.transition_controller.start_volume_fade_only(session, fade_sec=fade_sec)
                ...
                if dialog_trigger_time <= now < session_start and session_id not in self.triggered_dialog_sessions:
                    ...
                    if self.transition_controller.state == TransitionState.IDLE:
                        self.transition_controller.start_transition_sequence(session, fade_sec=fade_sec, countdown_sec=countdown_sec)
                    elif self.transition_controller.state == TransitionState.FADING:
...
class TransitionController(QObject):
    ...
    def start_volume_fade_only(self, session_payload: dict, fade_sec: int = None):
    ...
    def start_transition_sequence(self, session_payload: dict, fade_sec: int = None, countdown_sec: int = None):
        if self.state != TransitionState.IDLE:
            return
    ...
    def complete_transition(self, user_inputs: dict):
        ...
        try:
            if self.volume_manager.volume_interface:
                self.volume_manager.volume_interface.SetMasterVolumeLevelScalar(0.0, None)
